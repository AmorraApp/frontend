...
export default class MaskUtils {
  ...
  getStringFillingLengthAtPosition (string, position) {
    ...
    const insertedValue = characters.reduce((value, character) => this.insertCharacterAtPosition(value, character, value.length), Array(position + 1).join(' '));
  ...
  insertStringAtPosition (value, string, position) {
    ...
    value = characters.reduce((val, character) => this.insertCharacterAtPosition(val, character, val.length), value.slice(0, position));
    ...
    if (isFixedLength) {
      ...
      value = editableCharactersAfter.reduce((val, character) => {
        ...
        if (!this.isPositionEditable(val.length)) {
          val += mask.slice(val.length, nextEditablePosition).join("");
        }

        return this.insertCharacterAtPosition(val, character, val.length);
      }, value);
    }
